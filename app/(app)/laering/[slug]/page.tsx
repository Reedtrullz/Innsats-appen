...
export function TrainingPathDetail({ path, cards, sources }: { path: TrainingPath; cards: ActionCard[]; sources: SourceDocument[] }) {
  ...
  const pathSources = path.sourceIds.map((sourceId) => sourceById.get(sourceId)).filter((source): source is NonNullable<typeof source> => Boolean(source));
    ...
    <article className="space-y-4">
            ...
            {cardsForPath.map((card) => (
              <li key={card.slug} className="rounded-2xl border border-slate-200 p-3">
                ...
                {(card.competenceRequired ?? []).length > 0 ? (
                  <p className="mt-1 text-xs font-semibold text-slate-600">Kompetansekrav: {(card.competenceRequired ?? []).join(', ')}</p>
                ) : null}
              </li>
            ))}
          </ul>
      ...
      <section className="rounded-3xl bg-white p-5 shadow-sm">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {pathSources.map((source) => <SourceBadge key={source.id} source={source} withAnchor />)}
        </div>
      </section>
    </article>
  );

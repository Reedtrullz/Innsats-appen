        ...
        <ul className="space-y-2">
          {linkedCards.map((card) => (
            <li key={card.slug} className="rounded-2xl border border-slate-200 p-3">
              ...
              {(card.competenceRequired ?? []).length > 0 ? <div className="mt-2 flex flex-wrap gap-2 text-xs font-bold text-slate-700">{(card.competenceRequired ?? []).map((competence) => <span key={competence} className="rounded-full bg-slate-100 px-2.5 py-1">{competence}</span>)}</div> : null}
            </li>
          ))}
        </ul>
        ...
        {ordered.map((path) => (
          <article key={path.slug} className="rounded-3xl bg-white p-4 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div><p className="text-sm font-black uppercase tracking-wide text-sky-800">{path.courseCode}</p><h2 className="text-2xl font-black"><Link className="text-slate-950 hover:text-sky-800" href={`/laering/${path.slug}`}>{path.title}</Link></h2></div>
            ...
            <dl className="mt-3 grid gap-3 text-sm sm:grid-cols-3">
              <div className="rounded-2xl bg-slate-50 p-3"><dt className="font-black">Målrolle</dt><dd>{path.targetRoles.map((role) => roleLabels[role]).join(', ')}</dd></div>
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <div className="rounded-2xl border border-slate-200 p-3"><h3 className="font-black">Ferdigheter</h3><ul className="mt-2 flex flex-wrap gap-2 text-sm text-slate-700">{path.skills.map((skill) => <li key={skill} className="rounded-full bg-slate-100 px-2.5 py-1">{skill}</li>)}</ul></div>
              <div className="rounded-2xl border border-slate-200 p-3"><h3 className="font-black">Kilde-ID-er</h3><ul className="mt-2 space-y-1 text-sm text-slate-700">{path.sourceIds.map((sourceId) => <li key={sourceId}>{sourceId}</li>)}</ul></div>
            </div>

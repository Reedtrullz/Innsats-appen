...
export function TilfluktsromModuleContent({ cards, checklists, protectionMeasures }: { cards: ActionCard[]; checklists: OperationalChecklist[]; protectionMeasures: ProtectionMeasure[] }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="rounded-3xl bg-white p-4 shadow-sm">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2">{workflow.map((item) => <article key={item.title} className="rounded-2xl border border-slate-200 p-3"><h3 className="font-black">{item.title}</h3><p className="mt-1 text-sm text-slate-700">{item.text}</p></article>)}</div>
      </section>
      <section className="rounded-3xl bg-white p-4 shadow-sm">
        <h2 className="text-xl font-black">Godkjente tiltak og ansvar</h2>
        <div className="mt-3 space-y-3">{publicMeasures.map((measure) => <article key={measure.slug} className="rounded-2xl border border-slate-200 p-3"><h3 className="font-black">{measure.title}</h3><p className="mt-1 text-sm text-slate-700">Ansvar: {measure.responsibleAuthority}</p>{measure.dataWarnings.map((warning) => <p key={warning} className="mt-2 text-sm font-semibold text-amber-900">{warning}</p>)}</article>)}</div>
      </section>
      <section className="rounded-3xl bg-white p-4 shadow-sm">
        <h2 className="text-xl font-black">Sjekklister</h2>
        <div className="mt-3 space-y-3">{tilfluktsromChecklists.map((checklist) => <article key={checklist.slug} className="rounded-2xl border border-slate-200 p-3"><h3 className="font-black">{checklist.title}</h3>{checklist.warning ? <p className="mt-1 text-sm font-semibold text-amber-900">{checklist.warning}</p> : null}<ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-slate-700">{checklist.items.slice(0, 5).map((item) => <li key={item.id}>{item.label}</li>)}</ul></article>)}</div>
      </section>
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-3"><h2 className="text-xl font-black">Tiltakskort</h2><Link className="inline-flex min-h-11 items-center rounded-full bg-sky-50 px-4 text-sm font-bold text-sky-800" href="/sok?intent=action">Finn alle tiltak</Link></div>
